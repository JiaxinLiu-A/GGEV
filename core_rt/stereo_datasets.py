...
def fetch_dataloader(args):
    ...
    if args.train_datasets == 'sceneflow':
    ...
    elif args.train_datasets == 'kitti_finetune':
        ...
        kitti15 = KITTI(aug_params, year=2015)
        logging.info(f"Adding {len(kitti15)} samples from KITTI 2015")
        new_dataset = vkitti2 + kitti12 * 50 + kitti15 * 50
        logging.info(f"Adding {len(new_dataset)} samples from KITTI Mixture Dataset")
