...
@torch.no_grad()
def validate_eth3d(args, model, iters=32, mixed_prec=False):
    ...
    f = open('%s/test_ETH3D.txt' % args.logdir, 'a')
    f.write("Validation ETH3D: %f, %f\n" % (epe, d1))
    print("Validation ETH3D: EPE %f, D1 %f" % (epe, d1))
    return {'eth3d-epe': epe, 'eth3d-d1': d1}


@torch.no_grad()
def validate_kitti(args, model, iters=32, mixed_prec=False):
    ...
    f = open('%s/test_KITTI.txt' % args.logdir, 'a')
    f.write("Validation KITTI: %f, %f\n" % (epe, d1))
    return {'kitti-epe': epe, 'kitti-d1': d1}


@torch.no_grad()
def validate_sceneflow(args, model, iters=32, mixed_prec=False):
    ...
    f = open('%s/test_sceneflow.txt' % args.logdir, 'a')
...
@torch.no_grad()
def validate_middlebury(args, model, iters=32, split='MiddEval3', resolution='F', mixed_prec=False):
    ...
    f = open('%s/test_middlebury.txt' % args.logdir, 'a')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore_ckpt', help="restore checkpoint", default='./pretrained_models/ggev/sceneflow.pth')
    parser.add_argument('--logdir', default='./eval', help='the directory to save logs and checkpoints')
    ...
    parser.add_argument('--encoder', default='vits', help="DepthAnything V2 encoder.")
    ...
    if args.dataset == 'eth3d':
        validate_eth3d(args, model, iters=args.valid_iters, mixed_prec=args.mixed_precision)

    elif args.dataset == 'kitti':
        validate_kitti(args, model, iters=args.valid_iters, mixed_prec=args.mixed_precision)

    elif args.dataset in [f"middlebury_{s}" for s in 'FHQ']:
        validate_middlebury(args, model, iters=args.valid_iters, resolution=args.dataset[-1], mixed_prec=args.mixed_precision)

    elif args.dataset == 'sceneflow':
        validate_sceneflow(args, model, iters=args.valid_iters, mixed_prec=args.mixed_precision)
